...
- **emp_no_receipt**: 无票报销，用户说"无发票""没有发票"等。**触发条件**（必须同时满足才返回此意图，否则不返回）：
  - 用户文本明确提到"无票"/"无凭证"/"没有发票"/"无票报销"等关键词
  - 用户提供了有效的**金额**（正数，单位元，0 < 金额 ≤ 100000），如"120 元"/"¥88.5"/"500"
  - 用户提供了有效的**用途描述**（≥2 个汉字或单词，明确说明费用类型/场景，如"打车费"/"客户招待餐费"/"办公文具采购"）
  - **用途描述必须为有意义的中文/英文实词短语**（如"打车费"/"住宿费"/"打印费"），不能仅为符号（如"???"/"!!!"）、不能仅为单字符（如"x"）、不能直接复用"无凭证报销"这一意图词作为用途
  - **不满足任一条件时不要返回此意图**，让助手自然引导用户补充（agent 模式 LLM 直接生成引导回复）
  - 反例（不返回 emp_no_receipt）：
    - "无凭证报销"（无金额无用途）
    - "无凭证报销 120 元"（无用途）
    - "无凭证报销打车费"（无金额）
    - "无凭证报销 -100 元打车费"（金额为负）
    - "无凭证报销 0 元打车费"（金额为零）
    - "无凭证报销 abc 元打车费"（金额非数字）
    - "无凭证报销 100 元 x"（用途过短）
    - "无凭证报销 100 元 ???!!!"（用途为符号）
  - 正例（返回 emp_no_receipt + amount + description）：
    - "无凭证报销 120 元打车费" → amount=120, description="打车费"
    - "我有一笔 350 元的客户招待餐费需要无票报销" → amount=350, description="客户招待餐费"
    - "无票报销 88.5 元办公文具采购" → amount=88.5, description="办公文具采购"
...
- **emp_mark_travel_day**: 员工描述出差日期/行程，标记为报销补贴用的出差日，如"8月15日出差""8月15-17日去北京""标记8月15为出差日""下周二出差"。槽位：travel_dates（数组，YYYY-MM-DD 格式，区间需展开）、note（可选，如"北京出差"）。注意：仅在用户明确说"出差""出差日"或具体日期+出差场景时返回此意图，单纯说"我下周二"不返回此意图
...
- **travel_dates**: 出差日期列表（字符串数组，仅emp_mark_travel_day有效）。每个元素为 YYYY-MM-DD 格式。支持单日、多日、区间三种表述：
  - "8月15日" → ["2026-08-15"]（用当前年份补全）
  - "8月15-17日" / "8月15到17号" → ["2026-08-15","2026-08-16","2026-08-17"]（展开区间）
  - "8月15 16 17" → ["2026-08-15","2026-08-16","2026-08-17"]
  - "2026-08-15" → ["2026-08-15"]
  - "下周二" → 用当前日期推算后填入对应 YYYY-MM-DD
  - **必须展开为完整日期列表**，不要返回 ["2026-08-15/2026-08-17"] 这种区间字符串
- **note**: 出差备注（字符串，仅emp_mark_travel_day可选）。如"北京出差""返程""去上海谈客户"。无明确备注则不填
